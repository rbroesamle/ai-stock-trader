...
def delete_duplicate_articles(this_company):
    ...
    for i in range(0, len(this_company.relevant_articles)):
        for j in range(i + 1, len(this_company.relevant_articles)):
            if (
                calculate_normalized_osa(
                    this_company.relevant_articles[i].headline,
                    this_company.relevant_articles[j].headline,
                )
                > 0.6
            ):
                append = False
                break
        no_duplicate_articles.append(
            this_company.relevant_articles[i]
        ) if append else None
        append = True
...
class Filter:
    def __init__(self):
        self.companies = StocksManager().get_companies()
        ...
        for this_company in self.companies:
            for company_possible_name in this_company.possible_names:
                if company_possible_name in article.text:
                    company_names_in_article.append(
                        this_company
                    ) if this_company not in company_names_in_article else None
